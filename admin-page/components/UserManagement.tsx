...
export default function UserManagement() {
  ...
  const prodiOptions = [...new Set(entries.map((e) => e.prodi).filter(Boolean))] as string[];
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <input type="text" value={search} onChange={(e) => handleSearch(e.target.value)}
              ...
              className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-600 placeholder-gray-400 outline-none focus:ring-2 focus:ring-red-300 transition w-48" />
            ...
            <button
              ...
              className="flex items-center gap-2 border border-gray-200 text-gray-600 hover:bg-gray-50 text-sm font-medium px-3 py-1.5 rounded-lg transition disabled:opacity-50"
            >
              {uploading
              ...
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i} className="animate-pulse border-b border-gray-50">
                    <td className="px-4 py-4"><div className="w-4 h-4 bg-gray-100 rounded" /></td>
                    <td className="px-4 py-4"><div className="w-6 h-4 bg-gray-100 rounded" /></td>
                    <td className="px-4 py-4"><div className="w-36 h-4 bg-gray-100 rounded" /></td>
                ...
                <tr><td colSpan={colSpan} className="px-6 py-12 text-center text-gray-400">
                  {search || filterProdi ? "Tidak ada hasil yang cocok" : `Belum ada data ${tabLabel.toLowerCase()}`}
                </td></tr>
              ) : (
                paginated.map((entry, idx) => (
                  <tr key={entry.id} className={`transition ${selected.has(entry.id) ? "bg-red-50" : "hover:bg-slate-50/80"}`}>
                    ...
                    <td className="px-4 py-4 text-gray-500">{(page - 1) * PAGE_SIZE + idx + 1}</td>
                    <td className="px-4 py-4 text-gray-800 font-medium">{entry.nama}</td>
